...
console.log(`%d Busacnado los comentarios del postId: ${postId}...`,"color: cyan; font-weight:bold");
  try {
    ...
    // Dentro, imprime solo el 'email' de cada comentario para verificar el tipado.
     data.forEach(element => {console.log(element.email);});

  } catch (error) {
    // 7. [ERRORES]: Captura el error y muéstralo con console.error.
    console.error("Fallo al intenatr obtener los comentarios...",error)
  }
};
